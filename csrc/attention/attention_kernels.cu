...
__global__ void single_query_cached_kv_attention_quantized_kernel(
  ...
  using Vec_dequant = typename FloatVecTemp<Vec_quant>::Type;
...
#pragma unroll
      for (int j = 0; j < NUM_VECS_PER_THREAD; j++) {
        ...
        Vec_dequant k_vec_dequant = dequant(k_vec_quant);
...
#pragma unroll
  ...
  using V_vec_dequant = typename FloatVecTemp<V_vec_quant>::Type;
  using L_vec = typename Vec<scalar_t, V_VEC_SIZE>::Type;
  using Float_L_vec = typename FloatVec<L_vec>::Type;
...
#pragma unroll
    for (int i = 0; i < NUM_ROWS_PER_THREAD; i++) {
      ...
      if (row_idx < HEAD_SIZE) {
        ...
        V_vec_dequant v_vec_dequant = dequant(v_vec_quant);
...
#define LAUNCH_ATTENTION_QUANTIZED_KERNEL(T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS)                        \
  cudaFuncSetAttribute(                                                                       \
      vllm::single_query_cached_kv_attention_quantized_kernel<T, uint8_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS>,   \
      cudaFuncAttributeMaxDynamicSharedMemorySize, shared_mem_size);                          \
  vllm::single_query_cached_kv_attention_quantized_kernel<T, uint8_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS>        \
  <<<grid, block, shared_mem_size, stream>>>(                                                 \
    out_ptr,                                                                                  \
